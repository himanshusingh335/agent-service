...
`tui/client/api_client.py` (`AgentServiceClient`) is a thin synchronous httpx wrapper; `stream()` parses raw SSE `data:` lines itself rather than using an SSE client library. `tui/client/main.py` starts with a top-level menu (`chat` / `logs` / `session` / `exit`) instead of dropping straight into a REPL. `chat` generates a fresh session id (`uuid4`) per chat and returns to the menu on `exit`/`quit` rather than ending the process — there's still no session persistence across separate `chat` runs. `logs` and `session` prompt for a session id typed in manually (there is no backend endpoint to list existing sessions) and call `GET /chat/{session_id}/logs` / `GET /chat/{session_id}/history` respectively. Human-in-the-loop approval (`_handle_pending` in `main.py`) is shared by both response modes inside `chat` and is unaffected by the menu restructuring.
